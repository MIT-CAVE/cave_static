...
const NumberField = ({
  ...
  const handleValueCommitted = (newValue, event) => {
    ...
    // REVIEW: Perf is degraded after `NumberFormat.format`
    // onChangeCommitted(event, NumberFormat.format(newValue, numberFormat))
    const clampedValue = Math.min(max, Math.max(min, newValue))
  ...
  return (
    ...
    >
      ...
      <BaseNumberField.Input
        render={(props, state) => {
          console.log(state.inputValue, typeof state.inputValue)
          return (
            <OutlinedInput
              {...{ id }}
              inputRef={props.ref}
              value={state.inputValue}
              // value={NumberFormat.format(state.inputValue, numberFormat)}
              {...{ label, color, placeholder, readOnly }}
              sx={[
                spinner === 'left' && { pl: 0 },
                spinner === 'right' && { pr: 0 },
              ...
              startAdornment={
                ...
                >
                  {spinner === 'left' ? (
                    <Spinner side="left" {...{ decreaseIcon, increaseIcon }} />
              ...
              endAdornment={
                ...
                >
                  ...
                  {spinner === 'right' || spinner === true ? (
                    <Spinner side="right" {...{ decreaseIcon, increaseIcon }} />
                  ) : spinner === 'leftAndRight' ? (
                    <SpinnerIncreaseButton
                      icon={increaseIcon}
                      // {...{ size }}
